...
if ((isset($_GET['action']) == "save")){
  include_once('uploadclass1.php');
    $title=$_POST['title'];//商品名
	$route=$_POST['route'];//商品
	$uid=$_POST['uid'];//发布者
	$time=date('Y-m-d') ;
	$pic1=$uploadfile1;
	$pic2=$uploadfile2;
	$pic3=$uploadfile3;
	$pic4=$uploadfile4;
	$sql="insert into `town_shopping`(title,route,uid,pic1,pic2,pic3,pic4,time)values('$title','$route','$uid','$pic1','$pic2','$pic3','$pic4','$time')";
    $result=mysql_query($sql,$conn);
   if($result){
  echo"<Script>window.alert('商品添加成功');location.href='subject_line_list.php'</Script>";
 }else{
	  echo"<Script>window.alert('商品添加失败');location.href='subject_line_add.php'</Script>";
  }
}
